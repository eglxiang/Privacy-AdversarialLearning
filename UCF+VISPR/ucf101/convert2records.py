...
def memoryhog(folder):
    directory = 'data'
    print(os.path.join('UCF-101_shuffled', folder))
    X_train, Y_train, X_test, Y_test = video_processing(videos_dir=os.path.join('UCF-101_shuffled', folder))
...
if __name__ == "__main__":
    ...
    if not os.path.exists('data'):
        os.makedirs('data')
    Parallel(n_jobs=num_cores)(delayed(memoryhog)(q) for q in next(os.walk('UCF-101_shuffled'))[1])

...
def conv2d(x, input_filters, output_filters, kernel, strides, mode='REFLECT', name='weights'):
    with tf.variable_scope('conv'):

        shape = [kernel, kernel, input_filters, output_filters]
        weight = tf.get_variable(initializer=tf.truncated_normal(shape, stddev=0.1), name=name)
...
def conv2d_transpose(x, input_filters, output_filters, kernel, strides):
    with tf.variable_scope('conv_transpose'):
        ...
        weight = tf.get_variable(initializer=tf.truncated_normal(shape, stddev=0.1), name='weight')
...
def residual(x, filters, kernel, strides):
    with tf.variable_scope('residual'):
        conv1 = conv2d(x, filters, filters, kernel, strides, name='weights_1')
        conv2 = conv2d(relu(conv1), filters, filters, kernel, strides, name='weights_2')

...
def memoryhog(folder):
    path = os.path.join("/home/wuzhenyu_sjtu/DAN_sbu/SBU/SBU_videos/clean_version/DEPTH", folder)
    print(path)
    train_lst, val_lst, test_lst = video_processing(path)
    convert_to_records(train_lst, 'training_clean_{}'.format(folder))
    convert_to_records(val_lst, 'validation_clean_{}'.format(folder))
    convert_to_records(test_lst, 'testing_clean_{}'.format(folder))
...
if __name__ == "__main__":
    ...
    num_cores = 5
    Parallel(n_jobs=num_cores)(delayed(memoryhog)(q) for q in next(os.walk('/home/wuzhenyu_sjtu/DAN_sbu/SBU/SBU_videos/clean_version/DEPTH'))[1])

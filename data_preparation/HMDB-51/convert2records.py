...
def write_video(videos, labels, width=160, height=120, nchannel=3):
    class_dict = {}
    with open('hmdbTrainTestlist/classInd.txt', 'r') as f:
        for line in f:
            print(line)
            words = line.strip('\n').split()
...
def memoryhog(folder):
    directory = 'data'
    print(os.path.join('hmdb51-shuffled', folder))
    X_train, Y_train, X_test, Y_test = video_processing(videos_dir=os.path.join('hmdb51-shuffled', folder))
...
if __name__ == "__main__":
    ...
    if not os.path.exists('data'):
        os.makedirs('data')
    Parallel(n_jobs=num_cores)(delayed(memoryhog)(q) for q in next(os.walk('hmdb51-shuffled'))[1])

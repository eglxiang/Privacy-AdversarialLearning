...
def write_video(X, Y_action, Y_actor, Y_setting):
    ...
    for i in range(len(X)):
        ...
        output = "SBU_videos/clean_version/{}_{}_{}.avi".format(Y_actor[i], Y_action[i], Y_setting[i])
...
for root, dirs, files in os.walk(path):
    ...
    if depth == 3:
        print(root)
        files = [file for file in files if file.startswith("rgb")]
        files.sort()
